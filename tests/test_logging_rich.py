...
from src.logging_rich import setup_logging
...
def test_demonstrate_rich_markup(console, logger):
    ...
    logger.info(
        "Status: [green]✓ Connected[/green] | Records: [blue]1,234[/blue] | Time: [magenta]1.5s[/magenta]")

    logger.warning(
        "Performance issue detected: Response time [red]2.5s[/red] exceeds threshold [yellow]2.0s[/yellow]")

    logger.info(
        "Available commands: [italic]start[/italic], [italic]stop[/italic], [italic]restart[/italic], [italic]status[/italic]")
    ...
    logger.info(
        "[bold]Operation completed:[/bold] [green]✓ Success[/green] ([italic]took 0.25 seconds[/italic])")
...
def test_demonstrate_structured_logging(console, logger):
    ...
    for i in range(0, total_items + 1, 25):
        ...
        if progress == 100:
            logger.info(
                f"[green]✓[/green] Processing complete: {i}/{total_items} items ([bold green]{progress:.0f}%[/bold green])")
        elif progress >= 75:
